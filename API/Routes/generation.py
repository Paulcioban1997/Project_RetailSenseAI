        ...
        if scaler is None or features is None or generator is None:
            ...
            for _ in range(count):
                row = {
                    ...
                    "customer_id": np.random.randint(100000, 999999),
                    "product_id": np.random.randint(10000, 99999),
                    "order_status": np.random.choice(["delivered", "shipped", "processing"]),
                    "customer_state": np.random.choice(["SP", "RJ", "MG", "BA", "RS", "CE", "SC"]),
                    "delivery_days": np.random.randint(1, 30),
                    "delay_days": np.random.randint(-5, 15),
                    "purchase_month": np.random.randint(1, 13),
                    "purchase_dow": np.random.randint(0, 7),
                    "total_price": np.random.uniform(10, 500),
                    "total_freight": np.random.uniform(0, 100),
                    "total_weight": np.random.uniform(0.1, 50),
                    "main_category": np.random.choice(["electronics", "home", "fashion", "sports"]),
                    "payment_value": np.random.uniform(10, 500),
                    "max_installments": np.random.randint(1, 13),
                    "payment_type": np.random.choice(["credit_card", "debit_card", "boleto"]),
                    "n_items": np.random.randint(1, 10),
                    "review_score": np.random.randint(1, 6),
                    "bad_review": np.random.choice([0, 1]),
                    "shipping_days": np.random.randint(1, 30),
                }
